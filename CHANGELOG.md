...
- **TEST-01:** Unit + integration test split — CI runs unit tests with `-race -shuffle=on` for determinism; separate integration test job with build-tag gate (`//go:build integration`)
