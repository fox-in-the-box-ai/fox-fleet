...
- Cloud-mode provisioning crash loop — `hermes.env` values containing spaces (CSP connect sources with both `https://` and `wss://` origins) were written unquoted, causing the Fox entrypoint to misparse them as shell commands (exit code 127); all values are now double-quoted
...
- Subdomain proxy sent `InstancePassword` instead of `PlaneAuthToken` as `X-Fox-Auth` header — Fox instances rejected all Fleet-proxied requests because the header value didn't match `FOX_PLANE_AUTH_SECRET`, causing users to see Fox's own login page instead of being transparently authenticated

## [1.6.2] - 2026-06-18

### Fixed

- Redirect loop on subdomain login when Fox instance requires its own authentication — Fleet no longer intercepts `/login` when a valid Fleet session exists; authenticated requests are proxied directly to Fox, letting Fox handle its own login flow for instance access
